...
kws_enh_task=true #是否执行kws_enh训练
...
enh_stats_dir="${expdir}/kws_stats_${fs}_min"
# The directory used for training commands
if [ -z "${enh_exp}" ]; then
enh_exp="${expdir}/enh_${enh_tag}_min"
fi
...
if ! "${skip_train}"; then
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        if $kws_enh_task; then
            _train_data_param+="--train_data_path_and_name_and_type ${_enh_train_dir}/wkp.scp,wkp_ind,text_int "
            _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/wkp.scp,wkp_ind,text_int "
        fi
    ...
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        ...
        if $kws_enh_task; then
            _train_data_param+="--train_data_path_and_name_and_type ${_enh_train_dir}/wkp.scp,wkp_ind,text_int "
            _train_shape_param+="--train_shape_file ${enh_stats_dir}/train/wkp_ind_shape "
            _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/wkp.scp,wkp_ind,text_int "
            _valid_shape_param+="--valid_shape_file ${enh_stats_dir}/valid/wkp_ind_shape "
            _fold_length_param+="--fold_length ${_fold_length} "
        fi
