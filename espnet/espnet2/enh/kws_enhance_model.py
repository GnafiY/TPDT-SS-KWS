...
class KWSEnhancementModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        wkp_ind = torch.tensor(torch.inf)
        if 'wkp_ind' in kwargs.keys():
            wkp_ind = kwargs['wkp_ind']
    ...
    def forward_enhance(
        ...
        if self.mask_module is None:
            input_mix = self.fusion_model(feature_mix)
            _, flens, others = self.separator(input_mix, flens, additional)
            # import pdb; pdb.set_trace()
            feature_pre = [feature_mix * m for m in others['mask']]
        else:
