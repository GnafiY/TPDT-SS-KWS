...
from espnet2.enh.kws_encoder.pooling import AttentiveStatsPool, TemporalAveragePooling
from espnet2.enh.kws_encoder.pooling import SelfAttentivePooling, TemporalStatisticsPooling

"""
    This module encode the condition list(if use), each audio will be encode to a single vector
"""
...
class EcapaTdnn(nn.Module):
    def __init__(self, input_size, channels=256, embd_dim=128, pooling_type="ASP"):
        super().__init__()
    ...
    def forward(self, x, masks):
        ...
        out = self.bn1(self.pooling(out,masks))
        out = self.bn2(self.linear(out))
        return out
    
if __name__ == '__main__':
    x = torch.rand(2,15,512)
    model = EcapaTdnn(512)
