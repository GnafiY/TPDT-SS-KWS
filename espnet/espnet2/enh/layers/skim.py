...
from espnet2.enh.kws_encoder.conformer import CrossEncoderLayer
...
class ConditionalSkiM(nn.Module):
    """Skipping Memory Net
    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            self.cross_attention.append(
                CrossEncoderLayer(128, 4, 128, 33, 0.1)
            )
        self.output_fc = nn.Sequential(
            nn.PReLU(), nn.Conv1d(input_size, output_size, 1)
        )
    ...
    def forward(self, input, condition):
        ...
        condition = condition.unsqueeze(0).repeat(B,1,1)
        
        output = input.view(B * S, K, D).contiguous()  # BS, K, D
        hc = None
        for i in range(self.num_blocks):
            output = output.view(B, S*K, D)
            # if i == self.adapt_layer or self.adapt_layer == -1:
            #     output = output * aux_embedding.unsqueeze(1).repeat(1, S*K, 1).view(-1, K, D).contiguous()
            output  = self.cross_attention[i](output, condition)[0] # B, SK, D
            output = output.view(-1, K, D).contiguous() #BS, K, D
