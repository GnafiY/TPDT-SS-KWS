    ...
    def __call__(
        ...
        if self.segmenting and lengths[0] > self.segment_size * fs:
            ...
            for i in range_(num_segments):
                ...
                if isinstance(self.enh_model, KWSEnhancementModel):
                    feats = self.enh_model.fusion_model(feats)
            ...
            if isinstance(self.enh_model, KWSEnhancementModel_v3):
                condition = self.enh_model.fusion_model(feats)
                feats, flens, others = self.enh_model.separator(feats, f_lens, condition)
            elif isinstance(self.enh_model, KWSEnhancementModel):
                input_mix = self.enh_model.fusion_model(feats)
                _, flens, others = self.enh_model.separator(input_mix, f_lens)
                feats = [feats * m for m in others['mask']]
            elif isinstance(self.enh_model, KWSEnhancementModel):
                logging.info('kws model inference')
                feats = self.enh_model.fusion_model(feats)
                feats, _, _ = self.enh_model.separator(feats, f_lens)
            else:
                feats, _, _ = self.enh_model.separator(feats, f_lens)
